...
const StyledModal = styled.div`
	...
	& #modal-center-dialog-container {
        ...
        `
			...
			${({ modalDisplay, pwdisupper, pwdislower }) =>
        modalDisplay == "signup" &&
        pwdisupper == true &&
        pwdislower == true &&
        `
...
export function Modal({
    ...
    switch (modalDisplay) {
      case "login":
        ...
        request(
          "noop",
          "get-jwt-token",
          JSON.stringify({ user: te, password: tp })
        );
        break;
      case "signup":
        setTrySignup(true);
  ...
  return (
    <StyledModal
      ...
      pwdisupper={hasUpper}
      pwdislower={hasLower}
      pwdisspecial={hasSpecial}
      pwdisnumeric={hasNumber}
    >
      <div
        id="modal-center-dialog-container"
        onClick={(e) => e.stopPropagation()}
      >
        <div id="modal-center-dialog-header">
          <div id="modal-center-dialog-header-text">{headerText} </div>
          <div id="close-modal" onClick={(e) => setModalShowing(false)}></div>
        </div>
        <div id="modal-center-dialog-content">
          {validcreds !== null && validcreds === false && (
            <Alerts
              type="danger"
              msg="Invalid Credentials. Please make sure your username and password are correct."
              showIcon={false}
            />
		  )
		 }
          ...
          {modalDisplay === "signup" &&
            ...
            trySignup === true && (
              <Alerts
                type="danger"
                msg="Password Validation Errors."
                ulErrorLogic={[
                  hasUpper,
                  hasLower,
                  hasNumber,
          ...
          { modalDisplay === "signup" && !userAvail && (
            <Alerts
              type="danger"
              msg="Email Account Exists..."
              showIcon={false}
            />
		  ) }
          {modalDisplay === "signup" && (
            <span className="modal-signup-text-input-fullname">
              <input
                type="text"
          ...
          <span className="modal-login-text-input-email">
            <input
              type="text"
              placeholder="Email"
              onChange={(e) => {
          ...
          <span className="modal-login-text-input-password">
            <input
              type="password"
              ...
              onChange={(e) => {
                ...
                if (modalDisplay === "signup") {
                  ...
                  waitTestPassword(e.target.value);
				}
          ...
          {modalDisplay === "signup" && (
            <div id="password-strength-indicator">
              <span className="pwd-has-upper-lower" />
              <span className="pwd-has-number" />
              <span className="pwd-is-special" />
              <span className="pwd-is-long" />
            </div>
          )}
          <div id="modal-submit-login" onClick={(e) => doRequest()}>
            {submitText}
          </div>
        </div>
      </div>
