...
export function NavDropMenu({ navDropMenuPosX, setNavDropMenuPosX, navDropMenuType , doLogOut }) {
  ...
  useEffect(() => {
		...
		switch(navDropMenuType) {
			case "resources":
			case "profile":
        url = `http://localhost:1200/rest/api/ui/navbar-drop-menu/${navDropMenuType}`;
        //url = `https://www.pr0con.com:1200/rest/api/ui/navbar-drop-menu/${navDropMenuType}`;
  ...
  return (
    ...
    >
      {jsonData != null &&
        jsonData.elements.map((el) => (
          <div className={el.type} onClick={(e) => doAction(el.action)}>
            <DynamicIcon svgIconUrl={`/icons/20px/${el.icon}.svg`} />
            {el.text}
          </div>
        ))}
    </StyledNavDropMenu>
